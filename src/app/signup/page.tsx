...
export default async function SignupPage({
  ...
  searchParams: { error?: string; check?: string; message?: string };
}) {
    ...
    <main className="bg-burgundy-dark pt-20">
      <section className="flex min-h-screen items-center justify-center py-24">
        <div className="w-full max-w-md border border-gold/40 bg-burgundy-dark/60 p-10">
          ...
          <h1 className="h-serif mt-3 text-4xl text-cream">Create Your Account</h1>

          {searchParams.check && (
            <p className="mt-4 border border-gold/40 bg-gold/10 px-4 py-3 text-sm text-gold-light">
              {searchParams.message || "Check your email to confirm your account."}
            </p>
          )}
          {searchParams.error && (
          ...
          <form action={signup} className="mt-8 space-y-5">
            <div>
              <label htmlFor="fullName" className="mb-2 block text-xs font-bold uppercase tracking-widest text-gold">
                Full Name
              </label>
